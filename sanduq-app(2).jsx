        ...
        {step===3 && (
          <div>
            ...
            <div style={{ display:"flex", gap:8 }}>
              ...
              <div style={{ flex:2 }}><PrimaryBtn onClick={onBack}>Create Sanduq 🎉</PrimaryBtn></div>
            </div>
          </div>
        )}
      </div>
...
function Onboarding({ onDone }) {
  ...
  if (step === "phone") {
    return (
      <div style={{ minHeight:"100vh", background:C.bg, padding:"52px 24px 40px", display:"flex", flexDirection:"column" }}>
        ...
        <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:26, fontWeight:800, color:C.text, marginBottom:8 }}>What's your number?</div>
        <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:14, color:C.textMid, marginBottom:28, lineHeight:1.5 }}>We'll text a code to verify it's you. No passwords to remember.</div>
        ...
        <div style={{ flex:1 }} />
        <PrimaryBtn onClick={()=>phoneValid&&setStep("code")} disabled={!phoneValid}>Send code</PrimaryBtn>
      </div>
    );
  }
  ...
  if (step === "code") {
    return (
      <div style={{ minHeight:"100vh", background:C.bg, padding:"52px 24px 40px", display:"flex", flexDirection:"column" }}>
        ...
        <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:14, color:C.textMid, marginBottom:28, lineHeight:1.5 }}>Sent to +1 {phone}. <span onClick={()=>setStep("phone")} style={{ color:C.blue, cursor:"pointer", fontWeight:600 }}>Change</span></div>
        ...
        <div style={{ fontFamily:"'DM Sans',sans-serif", fontSize:13, color:C.textDim, marginTop:18, textAlign:"center" }}>
          Didn't get it? <span style={{ color:C.blue, cursor:"pointer", fontWeight:600 }}>Resend in 0:24</span>
        </div>
        <div style={{ flex:1 }} />
        <PrimaryBtn onClick={()=>codeValid&&setStep("profile")} disabled={!codeValid}>Verify</PrimaryBtn>
      </div>
    );
  }
  ...
  return (
    <div style={{ minHeight:"100vh", background:C.bg, padding:"52px 24px 40px", display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ flex:1 }} />
      <PrimaryBtn onClick={()=>name.trim()&&onDone({ name:name.trim(), initials, color:avatarColor })} disabled={!name.trim()}>Start using Sanduq</PrimaryBtn>
    </div>
  );
}
...
export default function App() {
  ...
  const [sanduqs, setSanduqs] = useState(SANDUQS);
  ...
  if (!authed) return (
    <div>
      ...
      <Onboarding onDone={()=>{ setLoading(true); setAuthed(true); setTimeout(()=>setLoading(false),1100); }} />
    </div>
  );
  ...
  if (screen==="create") return (
    <div>
      ...
      <CreateScreen onBack={()=>setScreen("home")} />
    </div>
  );
        ...
        <div>
          ...
          <div style={{ padding:"24px 18px 0" }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
              ...
              <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                ...
                <button onClick={()=>setScreen("activity")} style={{ width:44, height:44, borderRadius:"50%", background:C.surface, border:`1px solid ${C.border}`, display:"flex", alignItems:"center", justifyContent:"center", position:"relative", cursor:"pointer" }}>
                  <svg width="19" height="19" viewBox="0 0 24 24" fill="none" stroke={C.textMid} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 3a6 6 0 0 0-6 6c0 4-1.5 6-2.5 7h17C19.5 15 18 13 18 9a6 6 0 0 0-6-6Z"/><path d="M10 20a2 2 0 0 0 4 0"/></svg>
                  {unread>0 && <div style={{ position:"absolute", top:8, right:9, width:8, height:8, borderRadius:"50%", background:C.green, boxShadow:`0 0 0 2px ${C.surface}` }} />}
                </button>
          ...
          <div style={{ padding:"0 18px" }}>
            ...
            {!loading && shown.map(g => {
              ...
              return (
                <div key={g.id} onClick={()=>setActiveGroup(g)} style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:22, marginBottom:16, cursor:"pointer", overflow:"hidden", transition:"transform .18s, border-color .18s" }}
          ...
          <button key={t.id} onClick={()=>setScreen(t.id)} style={{ flex:1, display:"flex", alignItems:"center", justifyContent:"center", background:"none", border:"none", cursor:"pointer", position:"relative" }}>
            {t.isAction
              ...
              : <div style={{ position:"relative", display:"flex", alignItems:"center", justifyContent:"center" }}>
                  ...
                  {t.id==="activity" && unread>0 && <div style={{ position:"absolute", top:-4, right:-6, minWidth:8, height:8, borderRadius:"50%", background:C.red }} />}
